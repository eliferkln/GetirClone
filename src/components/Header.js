...
function Header({ setShowModal, setShowModal1 ,setShowModal2}) {

	return (
		<>
		
			<div className="bg-purple-800 bg-brand-color px-8">
				
				<div className="h-14 sm:h-11 container mx-auto flex items-center justify-center sm:justify-between">
			
					<div className="flex gap-x-2 font-semibold self-end">
              ...
              className="text-lg text-white  transition-all text-opacity-80 px-4 hover:bg-sky-700 "
            > <Link to="/getir">
              getiryemek
			  </Link>
            </div>
            <div
              ...
              className="text-lg text-white font-semibold transition-all text-opacity-80 px-4 hover:bg-brand-hover-purple hover:rounded-t-lg"
            ><Link to="/getir">
              getirbüyük
			  </Link>
            </div>
					...
					<nav className="gap-x-8 hidden sm:flex">
						<div  onClick={()=> setShowModal1(true)}
						   className="hover:text-opacity-100 transition-colors text-white text-opacity-80 text-sm font-semibold gap-x-2 flex items-center">
							<FiGlobe size={18} onClick={()=> setShowModal1(true)}/>
							Türkçe (TR)
						</div>
						<div  onClick={()=> setShowModal(true)}
						   className="hover:text-opacity-100 transition-colors text-white text-opacity-80 text-sm font-semibold gap-x-2 flex items-center">
							<HiUser size={18} onClick={()=> setShowModal(true)}/>
							Giriş yap
						</div>
					
						<div  onClick={()=> setShowModal2(true)}
						   className="hover:text-opacity-100 transition-colors text-white text-opacity-80 text-sm font-semibold gap-x-2 flex items-center">
							<HiUserAdd size={19} onClick={()=> setShowModal2(true)}/>
							Kayıt ol
						</div>
					</nav>
				</div>
